...
| # | Tab | What it is |
|---|-----|------------|
| 1 | **📊 Slides** | 8-slide deck. Three slides drive cross-tab actions: Plan·Analyse (`⚡ Run plan in pi` + inline `↗ Open Cases`), Build·Execute (`⚡ Run build in pi`), and Test·Talk-to-it (3 example utterances each with copy + `⚡ Send to chat`, plus an `✨ Open the agent chat` CTA). Speaker-first design: minimal text on slides, the speaker tells the story. The pre-Plan and pre-Build narrator slides (`plan-review`, `build-intro`) were dropped — their content is delivered live by the speaker. |
| 3 | **✨ Agents** | Live agent chat. Lists every Active service agent (BotDefinition with at least one Active BotVersion) from the org, click one to open a chat session. Streams replies via Salesforce's `/einstein/ai-agent/v1/sessions/{id}/messages/stream` SSE endpoint through a server-side proxy. Requires Connected App credentials in `presentation/.env.local`; without them the panel shows a precise "missing env vars" empty state. |
...
| 4 | **⚡ Live demo** | `ghostty-web` terminal in an iframe, talking WebSocket→PTY to a `node-pty` shell rooted at the SFDX project. Toolbar has a **▶ Start pi agent** button to launch an interactive `pi` session manually; slide ⚡ Run buttons type `pi '<prompt>'` directly at the shell with the prompt pre-filled as argv. |
...
*Last update: **4-tab rig** (Slides / Planning / Agents / Live demo). 8 slides — each demo phase has one actionable slide: Plan·Analyse (with inline `↗ Open Cases`), Build·Execute, Test·Talk-to-it. The Test slide ships 3 example utterances (each with `caseId` + `customerEmail`) that the speaker can copy or pipe straight into the new Agents tab. The Agents tab lists every Active service agent in the org and opens a real streaming chat session against `/einstein/ai-agent/v1` (server-side OAuth client-credentials flow + SSE proxy) — see §3a + `presentation/.env.example`. Implementation guardrails for the build phase (deploy flags, agent_type, lightning__numberType, Request/Response inner classes, exact lifecycle tool args) live exclusively in `agentforce/AGENTS.md` where pi auto-loads them. Single-subagent / no-router topology enforced in BUILD_PROMPT and across the SFDX-root AGENTS.md. Planning tab defaults to a scoped view of `planning/` only, with a segmented toggle to flip into the whole SFDX project. `pi '<prompt>'` argv injection in the live terminal. Plan + Build + **Test** phases all shipped.*
